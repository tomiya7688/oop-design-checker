...
public static class UserInterfaceText
{
    ...
    )
    {
        ...
        return ruleId switch
        {
            ...
            "OOP106" =>
                "内部表現が外部へ直接公開され、所有objectを迂回して変更できる可能性があります。",
            ...
            "OOP304" =>
                "project-ownedな継承階層が深く、振る舞いの追跡が難しくなる可能性があります。",
            "OOP305" => "共有抽象で表現できる振る舞いに対して、具象型へ直接依存しています。",
            "OOP306" =>
                "置換可能なcollaboratorを利用側が直接生成しており、具象生成依存を避けられる可能性があります。",
            "OOP307" =>
                "この継承は、意味的なis-a関係よりcompositionで表現する方が適切な可能性があります。",
            "OOP401" =>
                "1つのclass内に、独立して存在できる複数のstate/behavior clusterがある可能性があります。",
            "OOP402" =>
                "stateを持つobjectから有意味なbehaviorが外部へ移りすぎている可能性があります。",
            "OOP403" =>
                "objectが自身のidentityと関係の薄い複数のsubsystemへ依存している可能性があります。",
            "OOP404" =>
                "別objectの内部graphを深く辿っており、内部構造へのcouplingが強くなっています。",
            "OOP405" =>
