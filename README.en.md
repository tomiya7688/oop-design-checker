...
> **English compatibility edition.** The Japanese [README.md](README.md) is the canonical document. If this translation differs from the Japanese version, the Japanese version takes precedence.
...
Implemented rules are defined in [the English compatibility rule reference](specification/en/check-rules.md) and include encapsulation, inheritance, polymorphism, visibility, static-state, operation complexity, object-integrity, dependency, and navigation checks.
...
Choose the UI language with `--language ja|en`. Japanese is the default; English is the compatibility display.
...
2. Add the release entry to the canonical Japanese `CHANGELOG.md`, then update `CHANGELOG.en.md`.
...
The default `text` format stays concise. Machine-readable formats can be selected with `--format`:
...
- `text` - localized human-readable lines plus summary
- `json` - stable versioned diagnostic array and severity summary
...
The GUI uses Japanese by default and can switch to the English compatibility display. It accepts a target path and an optional configuration path, then displays the same diagnostics produced by the CUI.
...
- [Canonical Japanese README](README.md)
- [English changelog compatibility edition](CHANGELOG.en.md)
- [English design definition compatibility edition](specification/en/object-oriented-design.md)
- [English check rules compatibility edition](specification/en/check-rules.md)
